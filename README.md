...
### Логика транзакций
- **Все суммы положительные**: транзакции всегда имеют положительное значение
- **Тип определяется категорией**: 
  - Категория "Доходы" = доход
  - Все остальные категории = расход
- **Повторяющиеся транзакции**: поддерживается настройка повторения через поле `repeatTime`

### Статистика
- **Автоматический период**: если не указаны даты, используется текущий месяц
- **Кривая трат**: показывает средние и текущие траты по дням
- **Изменения баланса**: отображает изменения баланса по датам

### Безопасность
- **JWT аутентификация**: все API эндпоинты требуют валидный токен
- **Изоляция данных**: каждый пользователь видит только свои данные
- **Аудит токенов**: все созданные токены записываются в CSV файл

### Производительность
- **Пагинация**: списки транзакций поддерживают пагинацию
- **Фильтрация**: возможность фильтрации по категориям и датам
- **Автоматические бэкапы**: данные сохраняются каждые 24 часа
